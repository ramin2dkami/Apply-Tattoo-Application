...
/** Which figure the customer is placing on. Both are generated by
 *  assets/tools/build_figure.py from one set of anatomy tables (specs/009), and both
 *  use the same part ids, so a region selection survives switching between them. */
 ...
 *  `clampY` holds the end cross-section instead of returning null, which is what the
 *  warp wants: a tattoo hanging a little past the end of a part should keep its shape
 *  rather than have the overhanging rows silently dropped. */
export function edgesAt(sil: SilPoint[], y: number, clampY = false): [number, number] | null {
  if (!sil.length) return null;
  if (y < sil[0].y || y > sil[sil.length - 1].y) {
    if (!clampY) return null;
    const e = y < sil[0].y ? sil[0] : sil[sil.length - 1];
...
export function surfaceAt(
  parts: Surfaceable[], x: number, y: number, clampY = false,
): Surface | null {
  let best: Surface | null = null;
  let bestDist = Infinity;
  for (const p of parts) {
    const e = edgesAt(p.surface.silhouette, y, clampY);
    if (!e) continue;
    const cx = (e[0] + e[1]) / 2;
    const r = (e[1] - e[0]) / 2;
    if (r <= 1) continue;
